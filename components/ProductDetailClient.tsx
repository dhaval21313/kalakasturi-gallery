...
export default function ProductDetailClient({ product }: { product: Product }) {
  ...
  const isVeenaLady = product.id === 'veena-lady';

  const rightSidebar = (
    <div className="lg:col-span-5 lg:sticky lg:top-8 h-fit flex flex-col gap-6 text-white w-full">
      {/* Header description */}
      <div>
    ...
    <>
      {isVeenaLady ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14 items-start">
          {/* Left Column: Vertical Stack of Images (Lg: 7 Columns) */}
          <div className="lg:col-span-7 flex flex-col gap-8 w-full">
            {images.map((img, idx) => (
              <div
                key={idx}
                onClick={() => setLightboxImgIndex(idx)}
                className="relative aspect-[3/4] w-full rounded-3xl overflow-hidden bg-[#0A0A0A] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.8)] cursor-zoom-in group/image"
              >
                <Image
                  src={img}
                  ...
                  draggable="false"
                />
                <div className="absolute bottom-4 right-4 bg-black/60 p-2.5 rounded-full border border-white/10 backdrop-blur-md opacity-0 group-hover/image:opacity-100 transition-opacity z-20">
            ...
            {product.video && (
              <div className="relative aspect-[3/4] w-full rounded-3xl overflow-hidden bg-black border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.8)]">
                <video
                  ref={videoRef}
                  src={product.video}
                  autoPlay
                      ...
                      >
                        ...
                        <div className="absolute bottom-6 left-6 right-6 flex items-center justify-between bg-black/75 px-5 py-3 rounded-2xl border border-white/10 backdrop-blur-md z-20">
                          <button onClick={togglePlay} className="flex items-center gap-2 hover:text-[#C19A6B] transition-colors">
                            {isPlaying ? <Pause className="w-4 h-4 text-[#C19A6B] fill-current" /> : <Play className="w-4 h-4 text-[#C19A6B] fill-current" />}
                            <span className="text-[10px] uppercase tracking-wider font-semibold">{isPlaying ? 'Pause Overview' : 'Play Overview'}</span>
                          </button>
                        </div>
                      </motion.div>
                    )}
      ...
      {/* Fullscreen Lightbox Modal Overlay for Veena Lady */}
      <AnimatePresence>
        {lightboxImgIndex !== null && (
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-black/40 z-50">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-widest text-[#C19A6B]">
                  {product.title}
                </h3>
            ...
            <div className="flex-1 flex items-center justify-between px-4 md:px-8 relative select-none">
              ...
              {images.length > 1 && (
                <button
                  ...
                  className="w-12 h-12 rounded-full bg-white/5 hover:bg-[#C19A6B] hover:text-black border border-white/10 text-white flex items-center justify-center backdrop-blur-md transition-all duration-300 z-50 cursor-pointer shadow-lg"
                  aria-label="Previous image"
                >
            ...
            {images.length > 1 && (
              <div 
                className="flex items-center justify-center gap-3 py-6 bg-black/40 border-t border-white/5 z-50"
                onClick={(e) => e.stopPropagation()}
              >
                {images.map((img, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative w-12 h-16 rounded-lg overflow-hidden border transition-all duration-300 bg-neutral-900 ${
                        isActive 
                          ? 'border-[#C19A6B] scale-105 shadow-[0_0_10px_rgba(193,154,107,0.3)]' 
